...
void __zqra(int _M,
            int maxrank,
            double _Complex* _CU, int ld_CU, int _Crk,
            int* pnew_CU_ncols,
            double _Complex* _AU, int ld_AU, int _Ark,
            double _Complex alpha, double _Complex beta, double _Complex* qrtauA,
            flop_counter* flops){
    int info;
    ...
    //hc_printmat(_CU, _M, _M, ld_CU);
    double _Complex z_one = (double _Complex)1.0;
    if(gemm_print_mat){
...
void __zqrb(
            int _M,
            int maxrank,
            double _Complex* _CV, int ld_CV, int _Crk,
            int* pnew_CV_ncols,
            double _Complex* _AV, int ld_AV, int _Ark,
            double _Complex* _BU, int ld_BU,
            double _Complex* _BV, int ld_BV, int _Brk,
            double _Complex* qrtauB, double _Complex* AcolBcolT,
            flop_counter* flops){
    int info;
    /// B2 = ( (iA{2} * iB{2}') * iB{1}');
    ...
    double _Complex alpha = (double _Complex)1.0;
    double _Complex beta = (double _Complex)0.0;
...
void __zsvd(
            int _M,
            int maxrank,
            double _Complex* _CU, int ld_CU,
            double _Complex* _CV, int ld_CV, int _Crk,
            double _Complex* _U, int ld_U,
            double _Complex* _V, int ld_V, int* pnew_UVrk,
            int rank,
            double acc,
            double _Complex* _rA, double _Complex* _rB, double _Complex* _T, double* sigma, double* svdsuperb,
            flop_counter* flops
            ) {
    int info;
  ...
  296          info = -11
    ...
    for(k = 0; k < rank_V; k++){
        ...
        double _Complex zdiagval = ddiagval+0.0 * _Complex_I;
...
void __znewu(
             ...
             int ncols_qA,
             double _Complex* _CU, int ld_CU, int _Crk,
             double _Complex* _U,  int ld_U,  int _Urk,
             double _Complex* qrtauA,
             flop_counter* flops
             ) {
    int info = 0;
    int CU_nrows = _M;
        ...
        if(0){
            ...
            for(j=0;j<U_ncols;j++){
                for(i=0; i < CU_nrows; i++){
                    double _Complex val = _U[j*ld_U+i];
                    if(val != val){
...
void __znewv(
        ...
        int ncols_qB,
        double _Complex* _CV, int ld_CV, int _Crk,
        double _Complex* _V,  int ld_V,  int _Vrk,
        double _Complex* qrtauB,
        flop_counter* flops
        ) {
    int info;
    int CV_nrows = _M;
                 ...
                 )
{
    ...
    int _M = M; int _N = N;
    double _Complex* _CU = CU; int ld_CU = LDC;
    double _Complex* _CV = CV; int ld_CV = LDC; int* pnew_Crk = &new_Crk;
    double _Complex* _AU = AU; int ld_AU = LDA;
    double _Complex* _AV = AV; int ld_AV = LDA;
    double _Complex* _BU = BU; int ld_BU = LDB;
    double _Complex* _BV = BV; int ld_BV = LDB;
    int rank = rk;
    { // FIXME remove these extra braces
        ...
        int nb    = _M;//ASSUMPTION
        double _Complex* qrtauA = NULL;
        size_t qrtauA_nelm = nb;
        if(use_scratch){
            qrtauA = work;
        } else {
            qrtauA = malloc(qrtauA_nelm * sizeof(double _Complex));
        }
        assert(qrtauA != NULL);
        double _Complex* qrtauB = NULL;
        size_t qrtauB_nelm = nb;
        if(use_scratch){
            qrtauB = work + qrtauA_nelm;
        } else {
            qrtauB = malloc(qrtauB_nelm * sizeof(double _Complex));
        }
        assert(qrtauB != NULL);
        ...
        __zqra(_M, maxrk, _CU, ld_CU, _Crk, &CU_ncols, _AU, ld_AU, _Ark, alpha, beta, qrtauA, flops);
        ...
        //Warning:In this function, there are assumptions
        //on leading dimensions and number of rows/cols
        double _Complex* qrb_aubut = NULL;
        size_t qrb_aubut_nelm = maxrk * maxrk;
        if(use_scratch){
            qrb_aubut = work + qrtauA_nelm + qrtauB_nelm;
        } else {
            qrb_aubut = malloc(qrb_aubut_nelm * sizeof(double _Complex));
        }
        ...
        int new_UVrk;
        double _Complex* newU = NULL;
        size_t newU_nelm = ld_newU * maxrk;
        if(use_scratch){
            newU = work + qrtauA_nelm + qrtauB_nelm + qrb_aubut_nelm;
        } else {
            newU = malloc(newU_nelm * sizeof(double _Complex));
        }
        double _Complex* newV = NULL;
        size_t newV_nelm = nb * maxrk;
        
        if(use_scratch){
            newV = work + qrtauA_nelm + qrtauB_nelm + qrb_aubut_nelm + newU_nelm;
        } else {
            newV = malloc(newV_nelm *  sizeof(double _Complex));
        }
        assert(newU != NULL);
        assert(newV != NULL);
        double _Complex *svd_rA = NULL;
        ...
        if(use_scratch == 1) { 
            ...
            svd_rA = malloc(svd_rA_nelm * sizeof(double _Complex));
        }
        double _Complex *svd_rB = NULL;
        ...
        if(use_trmm == 0) {
            ...
            if(use_scratch == 1) {
                ...
                svd_T = malloc(svd_T_nelm * sizeof(double _Complex));
            }
        } else {
